...
class Game:
    def __init__(self, size: int = 4):
        ...
        # self.repeat_button = self.menu_ui.update(new_score = 0, new_high_score = 0)
        ...
        for event in pygame.event.get():
            if(event.type == pygame.QUIT):
                ...
                if event.key == pygame.K_UP:
                    ...
                    self.move_left()
            elif(self.win and event.type == pygame.MOUSEBUTTONDOWN and event.button == 1):
                continue_rect = self.continue_button
                if continue_rect.collidepoint(event.pos):
                    self.win = False
                    self.board_ui.update(matrix = self.matrix) #NECESARIO?
...
# LD_PRELOAD=/usr/lib/libstdc++.so.6 python -u "/home/papadedios/Documents/Repos/2048-ai/game/game2.py"
    ...
    def check_game_over(self):
        if(not self.first_time_won and any(4 in row for row in self.matrix)):
            ...
            self.continue_button = self.board_ui.win()
            self.win = True
